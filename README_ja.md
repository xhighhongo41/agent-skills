...
| `version-start` | 共通 | 新バージョンの開始手順。着手前に版の状態(未着手・着手済み・リリース済み、飛び越しや別プロジェクトの番号)を照合してから、プロジェクト文書の読解、コードベースの把握、ブランチ作成、実装計画書のドラフト作成まで行い、実装には入りません。確認事項はコードを読まなくても答えられる日本語で尋ねます。中断した設計は再開できます。 |
| `version-implement` | 共通 | 確定した計画を完了まで進めます。入場ゲートでの版の状態照合と回答照合、タスクごとの実装サイクル、自走と中断を判定する決定木で動きます。中断後は実装記録とコミットを突き合わせて正しいタスクから再開し、未コミットの作業は消しません。 |
| `version-release` | 共通 | リリース手順のチェックリスト。入場ゲートで未実装の版やリリース済みの版を止め、途中で止まったリリースは残りのゲートから再開します。バージョン上げ忘れの検出、リポジトリから派生して公開されるもの（パッケージ・コンテナのレジストリ、静的ホスティング、URL で配られるマニフェスト）が新しい版になるかの確認、文書追随の確認、検証、PR、マージ、タグ付与、リリースノートのドラフト作成まで。 |
| `skill-sync` | 共通 | 自マシンのどのエージェントにどのスキルのどの版が入っているかを一覧し、本リポジトリと比較して、選んだものだけを更新します。ただし**書き込むのは自エージェントのコンフィグディレクトリの中だけ**です。対象エージェントが決まっているスキルは、実行中のエージェントが対象かを確かめ、**対象外なら候補から外します**。版が同じでも中身が配布元と違う複製は「内容が違う」として知らせ、既定では更新しません。 |
| `council` | **OpenCode 専用** | 円卓合議の手順。**互いに異なるモデル**が同じ議題を独立に考え、互いの成果物をレビューし合い、自分の成果物に付いたレビューに各自が応答してから、メインセッションが統合して1つの結論にします。各参加者は3ステージを通じて同じセッションを保ち、途中で入手した資料は目録付きで共有します。 |
...
使ってください。導入済みの版を本リポジトリの `index.json` と突き合わせ、版が同じものは中身も照合して、
...
| バージョン | 変更内容 |
|---|---|
| **1.7.0** | `council` は、各参加者のサブエージェントのセッションを**3ステージを通じて保つ**ようになりました。相互レビューと応答の段階は、本論を書いたセッションへの続きの指示として送るので、読んだ資料と判断の根拠が文脈に残ります。継続できないときは同じ参加者の新しいインスタンスを起動し、自分の成果物を読んで追いつかせます。また、合議中に入手した外部資料（抄録・論文・マニュアル等）は**議題ごとの資料フォルダ**に置き、何をどこから取得したかの目録を付けるようにしました。参加者は探す前に目録を見るので、同じ資料を3回ダウンロードすることがなくなります。`skill-sync` は版に加えて**内容も照合**します。版は配布元と一致するのにファイルの中身が違う複製は「内容が違う」として、ファイル名と差分の規模とともに知らせ、1件ずつ確認するまで更新から外します。 |
| 1.6.0 | 版管理系の3スキルが、着手前に**版の状態を照合**するようになりました。`version-start`・`version-implement`・`version-release` は同じ7状態（未着手・設計中・設計済み・実装中・実装完了・リリース中・リリース済み）を、ブランチ・タグ・計画書・実装記録・進行文書から読み取って要約を表示し、期待どおりの状態なら確認を取らずに進みます。尋ねるのは食い違いがあるときだけです: 未リリースの下位の版を飛び越している、このプロジェクトのどの版でもない番号、すでに着手済みまたはリリース済みの版。また、それぞれが**自分の途中からの再開**を持ちます。`version-start` はブランチを作り直さずに書きかけの計画書を仕上げ、`version-implement` は実装記録とブランチのコミットを突き合わせて正しいタスクから続け（未コミットの作業は消しません）、`version-release` は PR・マージ・タグ・リリースノート・進行文書の痕跡が欠けている最初のゲートから再開し、済んだゲートはやり直しません。3スキルの確認の質問文は、コードを読まなくても答えられる日本語で書かれます。 |
