...
| `council` | **OpenCode only** | Runs a round table: several *different* models think about the same question independently, review each other's write-ups, each answers the reviews left on its own write-up, and the main session merges them into one decision. |
