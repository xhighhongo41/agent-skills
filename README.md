...
| `version-start` | any | Opens a new version: reads the project docs, surveys the codebase, creates the branch, and drafts an implementation plan. Asks its questions in plain language, with code names only as a footnote. Stops before implementing. |
...
| `version-release` | any | Runs the release checklist: version-bump detection, a check that everything published *from* the repo (package and container registries, static hosting, manifests fetched by URL) will carry the new version, doc sync, verification, PR, merge, tag, draft release notes. |
